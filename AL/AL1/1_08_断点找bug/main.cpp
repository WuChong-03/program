...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    ...
    while (Novice::ProcessMessage() == 0) {
        ...
        if (keys[DIK_SPACE]) {
            ...
            // 発射間隔の調整用クールタイムの計算
            //【修正前のコード】（クールタイムのリセットタイミングが不適切）
            // 発射処理より先に shootCoolTime をリセットしてしまうため、
            // 常に if (shootCoolTime <= 0) が false になり、発射できないことがある。
        ...
        for (int i = 0; i < 8; i++) {
            if (enemy[i].isAlive) {
                ...
                if (enemy[i].respawnTimer > 0) {
                    ...
                    enemy[i].respawnTimer = 0;      //一回復活したらrespawntimerが０になって、次回の復活timeも０になっちゃう
                    enemy[i].isAlive = true;
                }
            }
        }
                ...
                for (int j = 0; j < 32; j++) {
                    if (bullet[j].isShoot) {
                        if (enemy[i].radius + bullet[j].radius >=
                            ...
                            enemy[i].isAlive = false;   //ここのindexが逆です
